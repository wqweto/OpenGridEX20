...
- **M5 -- Editing (in progress):** the in-place text editor, the checkbox editor and wrapping cells, plus an event log recorder so a scenario asserts the event sequence it fires alongside its pixels. Dropdown, combo and calendar editors are still open.
